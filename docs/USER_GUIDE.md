...
Any chart or structure drawing in **Results** can be taken away:
right-click it for **Copy picture**, **Save picture…**, and — for a
structure drawing, which is vector — **Copy as SVG**. The same actions
appear whether Results is docked, popped out, or opened as a copy in its
...
| Charge | Gasteiger partial charges, charges at a chosen pH by Gasteiger or MMFF94, and **3D charges by EEM** (Bultinck 2002, H, C, N, O and F) **or QEq** (Rappé–Goddard 1991, λ = ½, its 16 Table I elements) **or Ionescu's EEM** (2013, Mulliken at 6-31G\* or 6-31G\*\*, for H, C, N, O, S and Ca; validated on protein fragments only, so every result says it is an extrapolation, and a sulfur bonded to oxygen is refused) on the stored conformer: they change with the geometry and need a conformer whose hydrogens are atoms. QEq refuses a molecule whose iteration does not settle or whose solution reaches a charge bound, and a silicon result notes that it disagrees with the 1991 paper's SiH₄ value. The conformer's net charge is spread over the molecule, so no atom keeps its drawn formal charge, and no protonation is applied |
