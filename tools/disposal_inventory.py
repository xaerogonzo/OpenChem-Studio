...
TESTS = ROOT / "tests"
#: The generated artifacts live beside the experiment they are evidence
#: for, not beside the script -- the shape `tools/build_sources_doc.py`
#: already has with `docs/SOURCES.md`.
INVENTORY_MD = ROOT / "benchmarks" / "disposal" / "inventory.md"
INVENTORY_JSON = ROOT / "benchmarks" / "disposal" / "inventory.json"
...
#: The recipe itself, as a sequence of call names. It may be PREFIXED
#: at a site that needs a `close()` or a `hide()` first -- 7 of the 64
#: original sequences were -- so consumers match it against the LAST
#: THREE steps of a run, never against the whole run.
CANONICAL = ["setParent", "deleteLater", "sendPostedEvents"]
...
def sequences_in(path: Path) -> list[dict]:
    ...
    return sequences_in_source(
        path.read_text(encoding="utf-8"),
        str(path.relative_to(ROOT)).replace("\\", "/"),
    )


def sequences_in_source(source: str, label: str) -> list[dict]:
    """Every disposal sequence in a source STRING.

    Split out from `sequences_in` so the walk can be exercised on
    constructed input -- the two-level shape `ui/visual_check.py` uses,
    where the predicates are tested on geometry a fixture builds rather
    than on whatever the application happens to contain today. A
    population assertion pinned to one real function's current shape
    rots the moment that function is refactored, which is exactly what
    happened when `dispose()` grew an `if`.
    """
                ...
                found.append(
                    {
                        ...
                        "line": run[0].lineno,
                        "subject": subject,
                        # The bare call names, beside the rendered steps.
                        # A consumer matching on `steps` compares against
                        # "setParent(None)" and friends, which is how the
                        # first version of the guard in
                        # `test_qt_object_disposal.py` came out VACUOUS --
                        # caught by its own population assertion, on its
                        # first run.
                        "names": [c.func.attr for c in run],
...
def main(argv: list[str]) -> int:
    ...
    if args.check:
        ...
        if stale:
            print(f"STALE: {', '.join(stale)} -- re-run tools/disposal_inventory.py")
            return 1
