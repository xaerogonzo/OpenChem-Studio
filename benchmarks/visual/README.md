...
| `calculator_visibility.json` | the launcher with Joback and Detonation hidden by default, the footer naming them, Settings > Calculators, the master toggle offering them, and Reset putting it back. Restores the setting it changes | the tick beside a row and the launcher reading two different rules for what is offered; the real-registry run is the only place the footer and the eliding button meet a docked column's width |
