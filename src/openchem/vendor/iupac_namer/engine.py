...
def _build_inorganic_canonical_index() -> dict[str, dict]:
    from openchem.vendor.iupac_namer.data_loader import _INORGANIC_CURATED_SMILES
    index: dict[str, dict] = {}
    sources = (
...
def _name_cyanic_ester_functional_parent(
    ...
    for atom in mol.GetAtoms():
        ...
        try:
            ...
            from openchem.vendor.iupac_namer.assembly import assemble as _assemble_cyanic
...
def _name_sulfamide_functional_parent(
    ...
    def _name_components(n_atom, components):
        ...
        for comp in components:
            ...
            from openchem.vendor.iupac_namer.assembly import assemble as _assemble_sulfamide
...
def _name_sulfinothioate_ester_functional_parent(
    ...
    try:
        ...
        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_sfthio
...
def _name_sulfonothioate_ester_functional_parent(
    ...
    try:
        ...
        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_sthio
...
def _name_sulfite_ester_functional_parent(
    ...
    def _name_r(anchor_atom, comp):
        ...
        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_sulfite
...
def _name_phosphite_ester_functional_parent(
    ...
    def _name_r(o_idx: int, anchor_idx: int, comp: frozenset[int]) -> str:
        ...
        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_phos
...
def _name_dichalcogen_fc(
    ...
    *,
    ...
    def _name_r(anchor_atom, comp, partner_idx_set):
        ...
        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_local
...
def _name_sulfonic_anhydride_functional_parent(
    ...
    def _name_acid_half(s_atom, r_anchor, r_comp, role):
        ...
        try:
            ...
            from openchem.vendor.iupac_namer.assembly import assemble as _assemble_anh
    ...
    from openchem.vendor.iupac_namer.assembly import _acid_to_adjective
...
def _name_biaryl_ring_assembly(
    ...
    from openchem.vendor.iupac_namer.types import FreeValenceInfo, SubstituentMethod
    from openchem.vendor.iupac_namer.assembly import assemble as _assemble_biaryl
            ...
            if shape_ok and has_non_oxygen_dbond and r_anchor_count == 1:
                try:
                    ...
                    if acid_mol is not None:
                        ...
                        from openchem.vendor.iupac_namer.assembly import assemble as _assemble_chal
    ...
    if (
        ...
        if acyl_ok:
            try:
                ...
                from openchem.vendor.iupac_namer.assembly import assemble as _assemble_acyl
...
def _collect_stereo_descriptors(
    ...
    """
    from openchem.vendor.iupac_namer.types import StereoDescriptor
...
def _name_simple_alkyl_x_radical(mol, strategy=None, session=None) -> str | None:
    ...
    if bo != 1:
        return None
    try:
        from openchem.vendor.iupac_namer.engine import _select_substituent_method, _fvi_elide_locant_one
        sub_method = _select_substituent_method(frag_mol, att_idx_sub)
        sub_fv = FreeValenceInfo(
            bond_orders=(1,),
            method=sub_method,
...
def _name_smiles_bound(smiles: str, strategy) -> str:
    ...
    from openchem.vendor.iupac_namer.perception.organometallic import (
    ...
    from openchem.vendor.iupac_namer.perception.fg.maingroup_oxoacids import (
    ...
    from openchem.vendor.iupac_namer.perception.fg.chalcogen_acid_modifiers import (
...
def _name_bound(
    ...
    if (output_form == OutputForm.STANDALONE
            and free_valence is None):
        from openchem.vendor.iupac_namer.perception.fg.acid_infix_composition import (
    ...
    if (output_form == OutputForm.STANDALONE
            and free_valence is None):
        from openchem.vendor.iupac_namer.perception.fg.cyclic_suffixes import (
...
def _generate_retained_plans(perception, mol, output_form, free_valence, strategy, session):
    ...
    from openchem.vendor.iupac_namer.natural_products import try_steroid_stem_name
    ...
    if match and _is_valid_retained_name_for_standalone(match):
        ...
        if output_form == OutputForm.SUBSTITUENT and OutputForm.SUBSTITUENT not in valid_forms:
            return
        from openchem.vendor.iupac_namer.types import RetainedMatch, RetainedPlan
        rm = RetainedMatch(
            name=match.get("name", ""),
            smiles=smiles,
...
def _free_valence_suffix_for_retained(
    ...
    """
    from openchem.vendor.iupac_namer.types import FREE_VALENCE_SUFFIXES as _FVS
...
def _heteroaryl_substituent_with_locant(
    ...
    from openchem.vendor.iupac_namer.ring_naming.numbering import compute_ring_numberings
    from openchem.vendor.iupac_namer.types import RingSystem
...
def _choose_salt_fragment_form(frag) -> "OutputForm":
    ...
    if charge < 0 and _carved_acid_anion_sites(frag_mol):
        from openchem.vendor.iupac_namer.perception.charge_perception import classify_charges
...
def _substituent_acid_anion_fgs(mol, free_valence) -> tuple:
    ...
    if mol is None:
        return ()
    from openchem.vendor.iupac_namer.perception.charge_perception import _acidic_anion_site_kind
...
class SubstitutivePath:
    ...
    def _name_parent_candidates(
        ...
        if candidate.type == "chain":
            ...
            for _atoms in atom_sets_to_try:
                ...
                from openchem.vendor.iupac_namer.types import CandidateParent as _CP
    ...
    def execute(
        ...
        from openchem.vendor.iupac_namer.isotope import collect_isotope_labels as _coll_iso
...
def _recompute_ring_unsaturation_name(named_parent, numbering) -> "NamedParent":
    ...
    from openchem.vendor.iupac_namer.data_loader import get_chain_stem
